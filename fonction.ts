...
// autre façon : 
function somme2(a: number, b: number): number {
    return a + b;
}
...
function somme4(...numbers: number[]): number {
    let result = 0;
...
function totalCaracteres(...strings: string[]): number {
    let result = 0;
...
function total(...values: (string | number)[]) {
    let result = 0;
